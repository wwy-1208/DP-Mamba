    ...
    def __init__(self, individual, n_vars, nf, target_window, head_dropout=0):
        ...
        if self.individual:
            ...
            for i in range(self.n_vars):
                self.flattens.append(nn.Flatten(start_dim=-2)) # Flatten P and D
                self.linears.append(nn.Linear(nf, target_window))
                self.dropouts.append(nn.Dropout(head_dropout))
        else:
            self.linear = nn.Linear(nf, target_window) # nf is the already flattened dimension
            self.dropout = nn.Dropout(head_dropout)

    def forward(self, x):
        # x: Input shape depends on the model calling it.
        # If called by SSTModel_WaveletMultiBranch (individual=False):
        # Input x should be [B, N, total_output_dim] where total_output_dim == self.nf

        if self.individual:
            x_out = []
            for i in range(self.n_vars):
                # Assumes input x is [B, N, P, D] for individual case
                z = self.flattens[i](x[:,i,:,:]) # Apply flatten for individual case
                if z.shape[-1] != self.nf:
                    # This check might need adjustment depending on expected input for individual=True
                     raise ValueError(f"Flatten_Head individual: Flattened dim {z.shape[-1]} != expected nf {self.nf}")
                z = self.linears[i](z)
                z = self.dropouts[i](z)
                x_out.append(z)
            x = torch.stack(x_out, dim=1) # [B, N, target_window]
        else:
            # --- MODIFICATION START ---
            # Input x is already [B, N, total_output_dim]
            # Directly check the input's last dimension

            if x.shape[-1] != self.linear.in_features:
                raise ValueError(f"Flatten_Head (non-individual): Input feature dim {x.shape[-1]} != "
                                 f"Linear layer input features {self.linear.in_features}")

            # Apply the linear layer directly to the input x
            x = self.linear(x) # [B, N, target_window]
            # --- MODIFICATION END ---
            x = self.dropout(x)
        return x # [B, N, target_window]
